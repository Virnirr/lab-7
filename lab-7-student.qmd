...
fish |>
  ...
  distinct()
```
From the data exploration, I found that there are a total of `1796` observation wtih missing data. The variable (column) with missing data is only `weight`.
...
# using scales = "free_x": https://ggplot2.tidyverse.org/reference/facet_grid.html
# controlling aesthetic evaluation after computed values in geom_bar: https://ggplot2.tidyverse.org/reference/aes_eval.html
# theme modification: https://ggplot2.tidyverse.org/reference/theme.html

fish |>
  # Data wrangling to get frequency of missing values by year
  filter(
    if_any(
      .cols = weight,
      .fns = ~ is.na(.)
  ...
  geom_bar(aes(fill = after_stat(count)), 
           show.legend = TRUE) +
  facet_wrap(~ variables, scales = "free_x") + # Limits vary across facets
  scale_fill_gradient(
    low = "lightblue", high = "darkblue",
    name = "Frequency"  # Label for the legend
  ) +
  labs(
    title = "Frequency of Missing Values in Fish Data by Section, Trip, and Year",
    x = "Year/Section/Trip",
    y = "",
  ) +
...
The mistake that you made in the above rescaling code is breaking the `DRY (Don't Repeat Yourself) rule`. Duplicating the code can make it messy and unreadable as it grows larger and larger. 
...
fish |> 
  ...
  labs(x = "New value of fish length_scaled (mm)") +
  scale_y_continuous(limits = c(0,4000))
```
...
# Getting all the variables in a tibble: https://tidyselect.r-lib.org/reference/all_of.html

# rlang with multiple arguments as vector (rlang::ensym(x)): https://psych252.github.io/psych252book/figures/tidyeval.pdf

rescale_column <- function(df, ...) {
  vars <- ensyms(...)  # Capture column names as symbols
